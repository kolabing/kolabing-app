...
|---|---|---|---|
| Business | Yes | Yes, €49.99/month or a 3-month plan | A venue or a product/service sponsor that wants community foot traffic and exposure. |
...
Businesses are venues (café, restaurant, bar, bakery, coworking, coliving, gym, salon, retail, hotel) or product/service sponsors. They are the paying side. Price: €49.99/month, or a 3-month plan. Registration and exploration are free; the subscription unlocks the two gated actions only.
...
- Track self-completing "general missions" on the app Missions screen (added 2026-06-27, v1 curation — `kolabing-v2` #49). See §7.4 below.
...
seeded by `SystemChallengeSeeder` (backend repo), exactly **18 are `app_visible = true`**
today — 5 attendee, 7 business, 6 community — each independently verified to use a live
trigger. The remaining seeded missions exist in the database (manageable from admin,
visible in the defaults matrix where relevant) but are deliberately held back from the
app surface until their triggers are wired or product decides to launch them.

The event/general separation is enforced in **three** places on the backend, all
filtering `trigger_action IS NULL` to keep general missions out of event-scoped
surfaces: `SystemChallengeController` (`GET /api/v1/challenges/system`),
`Admin\ChallengeDefaultsController` (the admin defaults matrix), and
`ChallengeService::listForEvent()` (`GET /api/v1/events/{event}/challenges`).
