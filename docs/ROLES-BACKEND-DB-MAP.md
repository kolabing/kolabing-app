...
| Whole surface off | `config('kolabing.public_kolabs.enabled')` — **false by default (2026-08-22)**. Read by the controller (404), both views' links, the homepage route (skips the query entirely) and the sitemap builder. `indexable` cannot re-open a disabled surface. |
...
## 22. Tickets and happenings — backend map (added 2026-08-22)

Rules in ROLES §7.5. This is the Kolab → attendee chain, link by link.

| Link | Code | Tables |
|---|---|---|
| Kolab agreed | `ApplicationService::accept()` | `applications`, `collaborations` |
| Happening created/refreshed | `CollaborationHappeningService::ensureFor()` | `events` (+ `collaborations.event_id`) |
| Seat taken | `EventSignupService::signup()` | `event_signups` |
| Ticket issued + emailed | `TicketService::issueAndSend()`, `EventTicketMail` | `event_signups.ticket_code` / `ticket_issued_at` / `ticket_emailed_at` |
| QR drawn | `TicketService::qrSvg()` → `App\Support\QrCode`, payload from `App\Support\TicketLink::admitUrl()` | — |
| Admitted | `TicketService::admit()` → `CheckinService::record()` | `event_checkins` |

**No tickets table, on purpose.** A ticket *is* the sign-up plus a code. A second row that
can disagree with `event_signups.status` about whether someone has a seat is a bug waiting to
happen — cancel the sign-up and a stale ticket would still scan.

**One identifier.** `ticket_code`, ten characters from an alphabet with no `O/0/I/1/L`. It is
what the QR encodes *and* what a doorkeeper types when the QR will not scan; both are the same
permission, so splitting them would only let the two paths drift (the same reasoning
`CheckinService` applies to `checkin_token` / `checkin_code`). Ten characters keeps the QR at
version 3 (29×29 — scannable across a room) with ~10^15 codes.

**Authorisation inverts, and this is the part to get right.** The pre-existing door has the
*attendee* scanning a code the host displays, so the attendee is authenticated
(`CheckinService::checkin()`). A ticket has the *host* scanning a code the attendee carries, so
the **host** is authenticated and the person admitted is someone else
(`TicketService::admit()` → `Event::isHostedBy()`). Holding a code is therefore not entry, and
not even the holder can admit themselves. `POST /tickets/{code}/admit` returns **409** for
"already used" rather than an error, because at a busy door double-scanning is normal and
"they're already in" is the useful answer.

**Both doors write the same row.** `CheckinService::record()` was extracted so the two entry
points cannot drift on consequences: one `event_checkins` row, arrivals broadcast,
`total_events_attended` incremented, badges re-checked, community points awarded, tiers
re-evaluated, missions progressed.

**Two fixes that were blocking the whole chain.**
1. `EventSignupService::signup()` refused any event with `community_id === null`. A
   collaboration cannot reliably have one — `events.community_id` points at the NF-6
   `communities` table, while a collaboration's community side is a *profile* — so **every**
   Kolab happening answered "sign-ups are not enabled here". The guard is now
   `community_id === null && visibility !== public`, which still excludes the portfolio/past-event
   rows it was written for (they default to `members`).
2. `CollaborationQrCodeController` created its event inline with
   `partner_name = $profile->display_name` — an attribute Profile does not have — so every
   generated event was hosted by the literal word "Partner". It now calls the happening service,
   which resolves the name off `business_profiles` / `community_profiles` / `profiles.name`.

**Endpoints (all additive):** `GET /me/tickets`, `GET /tickets/{code}`,
`POST /tickets/{code}/admit`. `TicketResource` embeds `qr_svg` inline — the door is exactly
where the network is worst, so a ticket needing a second request to become scannable is a
ticket that fails in a basement bar. `used_at` is read from `event_checkins`, never cached on
the ticket, so a ticket can never disagree with the attendance list.

**Mail.** `EventTicketMail` is queued (`ShouldQueue`) and deliberately does **not** embed the
QR: every reliable way to put a QR in an email is a raster attachment, and this app's encoder
draws SVG, which Gmail and Outlook strip. The email carries the code as text and a link to the
ticket page; the QR is drawn there, which is where a phone gets held up anyway. A failed send is
logged, never thrown — the seat and the ticket are real either way, which is also why
`ticket_emailed_at` is separate from `ticket_issued_at`: a bounce can be retried without changing
the code already in someone's inbox.

**The web surfaces (added 2026-08-22).** All Blade + Alpine on the app host, no new endpoint:

| Route | View | Reads |
|---|---|---|
| `/onboarding/attendee` | `webapp.onboarding-attendee` | `GET /cities`, `GET /lookup/community-types`, `GET /communities/discover`, `GET /handle/available` → `PUT /onboarding/attendee` |
| `/tickets` | `webapp.tickets` | `GET /me/tickets`; cancels via `DELETE /events/{event}/signup` |
| `/admit/{code}` | `webapp.admit` | `POST /tickets/{code}/admit` |
| `/dashboard` (attendee branch) | `webapp.dashboard` | `GET /me/tickets`, `GET /events/discover?city_id=` |

`kbShell()` gained `isAttendee` and `needsAttendeeOnboarding` (the tell is a missing
`handle` — the one field onboarding always writes and nothing else sets). The bounce to
onboarding lives in `loadShell()`, not in each page's `init()`, so a page added later
cannot forget it; it returns null, which every page already treats as "stop".

Nav visibility is an Alpine expression per item in `partials/sidebar.blade.php`, because
role is only knowable client-side. `!isAttendee` items render visible and hide once the
role is known (sellers are the majority, so a flash of extra beats a flash of missing);
attendee-only items carry `x-cloak` for the same reason inverted.

**Mobile parity** is deliberate and checked by `WebAppAttendeePanelTest`: the payload
shape mirrors `AttendeeOnboardingData.toPayload()` in the Flutter client — same four
steps, same required/optional split, same omit-when-empty rule, same 400 ms handle
debounce, photo as a `data:` URI.

**Timing.** The ticket is issued inside the sign-up transaction and emailed *after* it commits.
On a `sync` queue the mail would otherwise be built inside the transaction, so a rollback would
leave someone holding a ticket for a sign-up that does not exist.
## 23. The collaboration page on the web — backend map (BE-NF-45, added 2026-08-25)

**Route:** `Route::view('/collaborations/{collaboration}', 'webapp.collaboration-detail')`
in the `webapp` group (`routes/web.php`). The id is read off the path; the page holds
no server state. Reached from `/kolabs` (Active + Finished rows) and from the
dashboard's Upcoming cards, both of which were previously dead ends.

### 23.1 What the page calls

| Action | Endpoint | Notes |
| --- | --- | --- |
| Load | `GET /api/v1/collaborations/{id}` | `CollaborationService::findOrFail()` eager-loads both profiles, the kolab, feedbacks, completions and reviews — one request is enough |
| Start | `POST …/activate` | Either participant; `CollaborationPolicy::activate` also requires `canBeActivated()` |
| Confirm | `POST …/completion` | `{status: yes\|not_yet\|no, note?}` — resubmitting updates the caller's own row |
| Finish | `POST …/complete` | Gated by the confirmations above |
| Cancel | `POST …/cancel` | `{reason}`, min 20 chars — the page blocks the button below that so the 422 never happens |
| Review | `POST …/review` | 5-star format; idempotent server-side (returns 200 with the existing row) |
| Impact | `POST …/feedback`, `PUT …/feedback` | PUT only while the partner has not submitted |

Nothing new was added to `routes/api.php`. This is a client-side gap being closed.

### 23.2 The slice of `CollaborationResource` the page binds to

`status`, `scheduled_date`, `kolab_id`, `kolab.title`, `event_id`, `my_role`,
`creator_profile` / `applicant_profile`, `business_profile` / `community_profile`
(logo fallback only), `actions.{can_activate,can_complete,can_cancel}`,
`viewer_must_confirm_completion`, `own_completion.{status,note}`,
`partner_completion_status`, `own_feedback`, `partner_feedback`, `has_reviewed`,
`viewer_must_resubscribe`.

Pinned by `tests/Feature/Api/V1/CollaborationPanelContractTest.php` — by name, because
the failure mode is silent: a renamed field does not error, it renders an empty string.

### 23.3 Two traps that cost real time here

1. **`display_name`, not `name`.** `ProfileSummaryResource` exposes
   `id`, `user_type`, `display_name`, `avatar_url` — there is no `name`, no `handle`,
   and no nested `business_profile` / `community_profile`. Reading `name` yields
   `undefined` and *every* partner silently renders as the fallback word "Partner"
   with no link. `resources/views/webapp/kolabs.blade.php` already had this right;
   the page now resolves the partner the same way (compare `creator_profile.id`
   against `me.id`).
2. **`error_code` before `errorText()`.** The lifecycle endpoints return their context
   in the `errors` bag (`own_status`, `partner_status`, `pending_completion_from`),
   not validation messages. `window.kb.errorText()` flattens `errors` first, so it
   would show the user the literal string `no` or `completion_not_confirmed`. The page
   reads `json.error_code` (falling back to `json.errors.error_code`) and maps the
   three completion refusals plus `invalid_status_transition` to real sentences,
   using `errorText()` only as the last resort.

### 23.4 Role behaviour (unchanged, now visible on the web)

- **The §2.8 re-gate** is applied from `viewer_must_resubscribe` alone. That flag is
  computed in `CollaborationResource::viewerMustResubscribe()`: business **and** no
  active subscription **and** status is scheduled or active. Communities always get
  `false` (`isBusiness()` short-circuits), and a completed collaboration is never
  ongoing — so the review and feedback survive a lapse, as §4 requires. The page must
  never re-derive this from `user_type`; four tests in
  `CollaborationPanelContractTest` hold the four corners of that rule.
- **Feedback is role-shaped by the request class.** `StoreCollaborationFeedbackRequest`
  marks `stories_posted` / `revenue` as `prohibited` for a community and `benefits` as
  `prohibited` for a business. The page renders and sends only the caller's own set.

### 23.5 Deliberately not wired

`PUT/POST /collaborations/{id}/challenges`, the challenge bonus endpoints, and
`POST /collaborations/{id}/qr-code`. Challenges are in flight in separate PRs, and QR
generation belongs to the event surface — the page links to `/events/{event_id}` when
the collaboration already has a door and shows nothing when it does not.

### 23.6 Tests

- `tests/Feature/WebApp/WebAppCollaborationDetailTest.php` — 20 tests over the shipped
  page source (the panel has no build step, so the behaviour ships as source).
- `tests/Feature/Api/V1/CollaborationPanelContractTest.php` — 9 tests pinning the
  payload the page depends on, including the four §2.8 corners.

**Written:** `collaborations.status`, `collaborations.completed_at`,
`collaboration_completions`, `collaboration_feedback`, `collaboration_reviews` — all
through existing services. No schema change.
...
## 25. Event audiences + the challenge cap — backend map (BE-NF-47, added 2026-08-25)

### 25.1 One decision point, three shapes

`App\Services\EventSignupService::audienceRefusal(Event $event, array $state): ?string`
— `null` means allowed, otherwise a refusal code. Every caller answers through it:

| Caller | Shape it needs |
|---|---|
| `assertEligible()` | throws `DomainException($code)` on signup |
| `canAccess()` | boolean, detail view |
| `canAccessFromLoadedState()` | boolean, list view with state preloaded |

That consolidation *is* the change. The rule had three implementations before;
`canAccessFromLoadedState()` still exists — it must, or hydrating a page of
events becomes one query per row — but it now hands its loaded state to
`audienceRefusal()` instead of re-deriving the rule.

`$state` is `array{owner: bool, member: ?CommunityMember, follows: bool}`.

### 25.2 Refusal codes

`not_a_follower`, `not_a_member`, `not_an_active_member`, `tier_not_permitted`.

### 25.3 Enum and columns

- `App\Enums\EventVisibility` — `public`, `followers`, `members`, `active_members`, `tier`.
- `events.visibility` is **`varchar`, no check constraint**, default `'members'`
  (verified against production `information_schema`), so the two added values
  needed **no migration**.
- `events.tier_gate` — `json`, nullable. Null or `[]` means no tier gate.
- Activity window — `CommunityMember::ACTIVE_WINDOW_DAYS` (90) via
  `CommunityMember::isActiveMember()`, reading `community_members.last_attended_at`.

### 25.4 Preloading — the N+1 that isn't

`hydrateSummaries()` resolves once per page: owned community ids, active
memberships keyed by `community_id`, and **followed community ids** — one
`CommunityFollower::whereIn(...)` for the whole page.

### 25.5 Where the follows feed diverges

- `EventService::list()` (following filter) — `whereIn('visibility', [public, followers])`.
- `EventDiscoveryService::applyFollowingFilter()` — `baseQuery()` narrows to
  `public`, which is right for the city feed and wrong here, so an `orWhere`
  re-admits `followers`-visibility events for communities the viewer follows.

Deliberately NOT admitted in either: `members` / `active_members` / `tier`. See
ROLES §8.8 for why that under-shows to a member on purpose.

### 25.6 The challenge cap is gone

`events.max_challenges_per_attendee` — **null now means unlimited**, and null is
the default. `ChallengeCompletionService` skips the count query entirely when it
is null, and only enforces when it is not.

Migration `2026_08_23_180000_drop_default_challenge_cap` drops the `10` default
and nulls the rows still holding exactly `10` — only the value the old default
produced, because an organizer who set anything else chose it. The column stays.

**Production note (drift, now closed):** that migration was applied to production
(`migrations` batch 74) while its code was still unmerged on
`integration/all-open-prs`. Production therefore ran nulled rows through the old
`?? 10` fallback and stayed capped at exactly the number the migration existed to
remove. Nothing broke because the fallback existed. The class of problem — a
migration reaching production ahead of its code, because the repo's local `.env`
`DB_*` points at the production Postgres and a bare `php artisan migrate` writes
straight to it — is worth guarding against, not just noting.

**Written:** `events.max_challenges_per_attendee` (migration only). Every
audience check is a read.

---

## 26. The community identity mask — backend map (BE-FX-22, added 2026-08-25)

### 26.1 One rule, one place

`App\Support\CommunityIdentityMask::applies(?Profile $viewer, Profile $subject): bool`

It is a support class rather than a resource concern because more than one resource
needs the same answer, and two copies of a paywall condition is how one of them ends
up wrong:

| Endpoint | Resource |
|---|---|
| `GET /profiles/{id}` | `PublicProfileResource` |
| `GET /profiles/{id}/public-profile` | `CommunityPublicProfileResource` |
| `GET /communities/{id}/public-profile` | `CommunityPublicProfileResource` |

`SuggestionResource` keeps its own copy on purpose: the masked *shape* differs there
(it also withholds the counterpart's `id`, because a suggestion card is not addressed
by one). The condition is the same; what it produces is not.

### 26.2 The guard order IS the rule

```php
if ($viewer === null || ! $viewer->isBusiness()) return false;   // must stay first
if ($subject->user_type !== UserType::Community) return false;   // no business paywall
if ($viewer->id === $subject->id) return false;                  // never your own
return ! $viewer->hasActiveSubscription();
```

`Profile::hasActiveSubscription()` returns **false for every non-business**, so
testing the subscription alone — or before the role — masks every community and every
attendee viewer on the platform. Same trap as §18.6 and §12.5.
`PublicProfileMaskTest` pins it from four directions: community viewer, attendee
viewer, business viewing a business, and viewing your own profile.

### 26.3 What is withheld, and what must not be

| Withheld under the mask | Why |
|---|---|
| `display_name`, `avatar_url`, `logo_url`, `profile_photo` | §2.5 names the name and the logo |
| `handle` | resolves the identity on its own, here and at `kolabing.com/p/{handle}` |
| `about` | where a community names itself in prose |
| `instagram`, `tiktok`, `website` | §2.5 names contact; §4.2 calls it the strongest reason to register |
| `public_url` | built from the handle — it hands over the name in the field a mask is assumed to have covered |
| `recent_reviews` | each review names its reviewer and links to them: a directory of this community's partners |
| `gallery`, `photos`, `past_events`, `past_collaborations` | §4.2 withholds past-event detail and collaboration partner names |

**Kept, deliberately:** `id`, `user_type`, `type` / `community_type`, `city_name`,
`completed_kolabs_count`, `public_stats`, the aggregate `reputation`, verification
fields. That is what makes it a blur and not a block (golden rule 5). Unlike a
suggestion card, `id` stays — the caller addressed this resource *by* it.

`identity_masked` (bool) is on both payloads so a client can render the blur as a
sales moment rather than an empty page.

### 26.4 The client half

The panel's `/profiles/{id}` page reads `p.identity_masked` and nothing else — it
never re-derives the rule from the viewer's role, which is how a community viewer
would end up blurred. Withheld visibly (a `blur-sm` placeholder and `●●●●●●●●●●`,
both `aria-hidden`), plus a subscribe card pointing at
`/subscription?reason=profile`. Copy: `webapp.profile.masked_title` / `masked_body` /
`masked_cta` in en/es/ca.

**Mobile still draws its own blur** over `hideCreatorIdentity`, computed client-side
from the discovery payload. That is now belt-and-braces on the profile page rather
than the only gate — but Explore's own payload is still unmasked server-side, which
is **BE-FX-21** and remains open.

**Written:** nothing. Every field is masked at serialisation time and stored as-is,
so subscribing reveals with no backfill.

---

## 27. Multi-Kolab events on the panel — backend map (added 2026-08-27)

### 27.1 The gate

`GET /api/v1/me/organizer-entitlement` →
`{has_event_creator_entitlement, capability, granted_at, expires_at, source}`.

Backed by `organizer_entitlements` (capability `event_creator`, `revoked_at` null,
latest `granted_at` wins) via `Profile::hasEventCreatorEntitlement()`.

The panel keeps it in `kbShell()` as `organizerEntitlement`, with
`canCreateEvents` reading **only** `has_event_creator_entitlement === true`. Null
until the call lands, so the nav entry stays hidden rather than flickering in.
`loadOrganizerEntitlement()` returns early for an attendee — the capability cannot be
held by one, so the request is not worth making.

**Never gate this on `user_type` or on a subscription.** It is a maintainer grant like
`can_manage` (§12), and no subscription is consulted on this surface in either
direction.

### 27.2 What each panel page calls

| Page | Calls |
|---|---|
| `/multi-kolab-events` | `GET multi-kolab-events/me?per_page=15` (+`&page=` for more) |
| `/multi-kolab-events/{id}` | `GET multi-kolab-events/{id}` **and** `GET multi-kolab-events/{id}/dashboard`, in parallel |
| `/dashboard` | `GET multi-kolab-events/me?per_page=3`, inside `loadExtras()` |

`multi-kolab-events/me` scopes on `creator_profile_id`, so the list is the viewer's
own events by construction — the panel adds no filter of its own.

### 27.3 Two payloads, two questions

`GET multi-kolab-events/{id}` is what the event **is** (title, `value_summary`, city,
category, `event_date`, `date_mode`, `roles[]`). `GET .../{id}/dashboard` is where it
**stands** (`role_counts.{total,open,filled}`, and per role
`application_counts.{pending,shortlisted,accepted,declined,withdrawn}`).

The board endpoint is organizer-only and **403s for anyone else**. The page treats
that as "no counts", not an error: it keeps the event, falls back to the detail
payload's `roles[]`, and maps them into the board's shape with
`application_counts: null`. A viewer who is not the organizer therefore sees the roles
without numbers they should not have, and no error banner for a refusal that is
correct.

### 27.4 Status pills

Both pages use the panel's existing `window.kbStatus()` rather than a second helper —
its tone map gained `recruiting` / `open` / `shortlisted` → warn, `confirmed` /
`filled` → ok, `expired` → neutral. `expired` is deliberately **not** `bad`: the date
ran out without the roles filling, which is an ending, not a failure; `cancelled`
stays `bad` because somebody called it off. Labels are `status.*` in en/es/ca.

### 27.5 Why not `/events`

`/events` on the panel is the attendee happening (`events` rows — the door with a QR,
§22). A multi-Kolab event is a different object with a different audience, so it takes
`/multi-kolab-events`, matching the mobile client's `/multi-kolab-events/:id` so a
link pasted between the two resolves. `WebAppMultiKolabEventsTest` pins the
non-collision.

### 27.6 Not wired yet

Create, edit, `roles` (add/edit), applicant review, and `publish` / `confirm` /
`complete` / `cancel` all exist on the API and have **no panel caller**. The surface
is read-only in this pass; the write flows remain mobile-only.
...
## 29. Admin quick-add — listing-first onboarding (added 2026-09-11)

Extends §9 (admin operator surfaces). Daniel, 2026-09-11, explicit: a maintainer needs to list a
business/community sourced from outreach (an Instagram reply, e.g.) **before** its owner has ever
touched the app, then let them claim it by email.

| Route | Controller | Guard | Notes |
|---|---|---|---|
| `GET /admin/users/quick-add` | `Admin\ManagedUserController::quickAddForm` | `auth:admin` + `maintainer` | Renders the reduced form (§2.14): user_type, name, email, city_id, instagram, phone_number. No password field. |
| `POST /admin/users/quick-add` | `Admin\ManagedUserController::quickAddStore` | same | `App\Http\Requests\Admin\QuickAddProfileRequest` validates; redirects to `admin.users.edit` on success. |

**`ManagedProfileService::quickAdd()`** (new method, alongside the existing `create()`/`update()`):
creates a `Profile` with `Str::random(32)` as the password (never shown, never emailed — the owner sets
their own via the reset flow) and `email_verified_at = now()` (the listing is live immediately, not
pending verification). Calls the existing `upsertDetailProfile()` for the business/community detail
row, then **additionally sets `city_id`** on that detail row — `upsertDetailProfile()` has never set
`city_id` despite both `business_profiles` and `community_profiles` having the column (§2 schema); the
full `/admin/users/create` CRUD form has the same gap today, untouched by this change since Daniel only
asked for the quick-add path.

**The welcome email reuses the existing password-reset broker, not a new magic-link system.**
`Password::broker()->createToken($profile)` mints the same token type the ordinary forgot-password flow
uses; `AdminProfileWelcomeMail` (`app/Mail/`, `ShouldQueue`, markdown template
`resources/views/mail/admin-profile-welcome.blade.php`) embeds it as
`/reset-password?token=…&email=…` — the exact URL shape `AppServiceProvider::configurePasswordReset()`
already builds for a normal reset email. A quick-added owner therefore sets their password through the
identical page and `AuthService::resetPassword()` code path as anyone who clicked "forgot password";
there is no separate token table, expiry rule, or claim endpoint to maintain.

**Public-profile screenshot surface:** `admin/users/edit.blade.php` gained a "View public profile"
button (business/community only) linking `App\Support\PublicProfileLink::urlFor($profile)` — the same
`kolabing.com/p/{slug}` mechanism from §16/§17, which needs no new column (slug is computed from the
name + a UUID suffix). Lets a maintainer open the live listing and screenshot it to send back to the
business/community as proof the listing exists.

**Tested** (`tests/Feature/Admin/QuickAddProfileTest.php`, run against a real Postgres scratch DB —
this repo's `phpunit.xml` targets sqlite but the box has no `pdo_sqlite` driver): quick-add creates a
business with `city_id` set, a community without one, a non-maintainer is forbidden, a duplicate email
is rejected, the welcome email's reset token **actually authenticates** end-to-end via
`POST /reset-password` (not just "email was queued"), and the resulting profile's public slug resolves
back to the same row via `PublicProfileLink::resolve()`.

**Key invariant:** does not change who is paywalled or what a business/community can do (§2.7, §3.6
unaffected) — only how a profile comes to exist. `source = quickAdd` is not tracked as a distinct
column; a quick-added profile is indistinguishable from a self-registered one once its owner has set a
password, by design (§2.2/§3.2 self-registration and this path converge on the same `profiles` row
shape).

---

## 30. Welcome email — admin-editable templates + preview-before-send (added 2026-09-14)

Extends §29. Daniel 2026-09-14, two explicit asks in the same session: (1) outreach happens in
different languages, so sending needs a manual language choice, not an automatic guess; (2) "i don't
want them to get an unapproved email" — nothing may send without a maintainer seeing the exact real
render first.

**Schema:** new table `admin_welcome_email_templates` — `id` (uuid), `locale` (string(10), **unique,
free text** — not a fixed enum, so a maintainer can add a language the code has never heard of),
`label` (display name, e.g. "Español"), `subject`, `intro_markdown`, `next_steps_markdown`,
`footer_markdown` (three separately-editable CommonMark chunks), `is_active` (bool). Model
`AdminWelcomeEmailTemplate` (`app/Models/`). Seeded EN/ES/CA via `AdminWelcomeEmailTemplateSeeder`
(`firstOrCreate` by locale — safe to re-run, never overwrites a maintainer's live edit once the row
exists). **Not** wired into `DatabaseSeeder`'s auto-chain (that chain includes dev-only seeders); run it
once post-deploy like the other curated seeders in `docs/DEPLOY-RUNBOOK.md`:
`php artisan db:seed --class=Database\Seeders\AdminWelcomeEmailTemplateSeeder`.

**Security boundary — content vs. control, deliberately split.** The two action buttons ("View your
listing" / "Set your password") and the URLs they point to — including the one-time password-reset
token — are hardcoded in `AdminProfileWelcomeMail` / `resources/views/mail/admin-profile-welcome.blade.php`,
**never** part of the editable `*_markdown` fields. `WelcomeEmailTemplateRenderer::render()` does plain
`{{token}}` regex substitution (name/who only) on the stored strings — deliberately not
`Blade::render()` on DB content, even though only maintainers can write it, so an editable field is
never a code-execution surface and can never redirect a CTA.

**Routes** (`auth:admin + maintainer`, under `/admin`):

| Route | Controller | Notes |
|---|---|---|
| `GET /users/{profile}/welcome-email-preview` | `ManagedUserController::previewWelcomeEmail` | Renders the real `AdminProfileWelcomeMail` HTML with a placeholder token (`'preview-only-not-a-real-link'`) — nothing is queued, no token is minted. `PreviewWelcomeEmailRequest` validates `locale` against an **active** template. |
| `POST /users/{profile}/send-welcome-email` | `ManagedUserController::sendWelcomeEmail` | The actual send — mints a real `Password::broker()` token, queues `AdminProfileWelcomeMail` via `Mail::to($email)->locale($locale)->queue(...)`. Only reachable from the preview screen in the UI (a "Confirm & send" button), though the route itself has no server-side dependency on having hit preview first — the gate is the UI flow, not a token/state machine. |
| `GET/POST/PUT/DELETE /email-templates[/{emailTemplate}]` | `EmailTemplateController` (`Route::resource`) | Full CRUD for the language set itself. `locale` unique (ignoring self on update); free-text but regex-constrained to `[a-zA-Z-]+`. |

**`->locale($locale)` matters and was a real bug caught while testing.** The two button *labels* are
translated via `lang/{en,es,ca}/admin_mail.php` (`__('admin_mail.view_listing_button')` etc.) — they are
NOT stored on the template row, since they're part of the fixed/non-editable button markup. Without
`Mail::to()->locale($locale)`, `__()` resolves against the app's current default locale regardless of
which language a maintainer picked — confirmed live via `tinker` render before the fix: an ES-locale
email queued with `Mail::to()->queue()` (no `->locale()`) rendered "View your listing" / "Set your
password" in English while the rest of the body was correctly Spanish. `AdminProfileWelcomeMail` also
gained an `AdminWelcomeEmailTemplate $template` constructor param (was implicit/hardcoded English
before).

**`GET /users/quick-add`'s controller no longer auto-sends** — `ManagedProfileService::quickAdd()` only
creates the profile now; `sendWelcomeEmail()` is a separate public method, called only from the new
route. `/admin/users/{profile}/edit` gained a "Send welcome email" card: a `<select>` populated from
`AdminWelcomeEmailTemplate::where('is_active', true)`, submitting via **GET** to the preview route (not
POST — no state change from choosing a language, matching the read-only nature of a preview).

**Tested:** `tests/Unit/Services/WelcomeEmailTemplateRendererTest.php` (token substitution, unknown
tokens left visible rather than silently dropped), `tests/Feature/Admin/EmailTemplateManagementTest.php`
(full CRUD + non-maintainer 403 + locale-uniqueness-on-self-update regression guard),
`tests/Feature/Admin/QuickAddProfileTest.php` (quick-add sends nothing; explicit send works and is
locale-tagged; preview renders real content and queues nothing; both routes reject an inactive locale
and a non-maintainer; the reset-link round-trip still authenticates end-to-end through the new flow).

---

## 31. Users list — search, city filter, test-row hiding, city map (added 2026-09-14)

Daniel 2026-09-14: `GET /admin/users` was "a lot of stale tests and not UI/UX friendly ... to show
listed users per city." Presentation-only — no role, paywall, or permission changed.

**Filters, all query params on `GET /admin/users` (`ManagedUserController::index`):**

| Param | Effect |
|---|---|
| `q` | `email ILIKE %q%` OR `businessProfile.name`/`communityProfile.name ILIKE %q%` |
| `user_type` | exact match on `profiles.user_type` |
| `city_id` | matches a `businessProfile.city_id` or `communityProfile.city_id` |
| `show_test` | checkbox, default OFF — when off, excludes `is_test_user = true` (§2.11) rows AND an email/name `ILIKE %test%`/`%example.com` pattern match, to also catch legacy QA rows predating that column |

`is_active` stays deliberately unfiltered regardless of the above (unchanged from before this change) —
an admin who cannot see a switched-off account cannot switch it back on.

**City counts** (`ManagedUserController::cityCounts()`, private): two `groupBy('city_id')` counts, one
each on `business_profiles` and `community_profiles` (both `withoutGlobalScope(ActiveProfileScope::class)`,
same reasoning as the eager loads above — the sub-profile scope would otherwise hide a deactivated
profile's row from its own city's count), merged in PHP and keyed by **city id**, not name — a city can
carry listings while being `is_active = false` on the `cities` row itself (deactivated market, historical
data), which would drop it from the `is_active`-scoped dropdown list; keying by id means the map still
finds it instead of silently losing the count. Deliberately no `HAVING` on a computed alias: SQLite (this
repo's test driver) and Postgres (prod) disagree on that, same class of gap as BE-FX-23.

**Map:** `resources/views/admin/users/index.blade.php` ports the exact Leaflet-circles-by-count pattern
already live in `admin/crm/index.blade.php` (OpenStreetMap tiles, `webapp-assets/leaflet/`, radius
scaled by count, click a marker to re-filter). The coordinate dict is hand-maintained and mirrors
`CitySeeder::getActiveCities()` (Barcelona/Madrid/Valencia/Sevilla/Bilbao/Mexico City/Tallinn/Berlin/
Paris/Warsaw) — a city with a nonzero count but no entry in the dict renders as a plain "Other (no map
coords)" badge instead of disappearing. Note: the CRM map's own coordinate dict (§ not renumbered here)
predates the Valencia/Sevilla/Bilbao/Mexico City cities and does not have them either — same drift risk,
not fixed in this change since it wasn't the surface asked about.

**Tested:** `tests/Feature/Admin/ManagedUserIndexTest.php` (search, city filter, default test-hiding +
`show_test=1` override, an unflagged-but-example.com-emailed row also hidden by default, the map's
embedded JSON data for a city with a listing).

---
## 32. The city filter matches the city, not the spelling (BE-FX-60, added 2026-09-22)

**Symptom:** a Mexico City demo account opened Explore, filtered by Mexico City, and got nothing —
while two real CDMX businesses were live and published.

**Root cause — two vocabularies for one column.** A Kolab stores its city as a *name*, not a
`cities.id`: `kolabs.preferred_city` (varchar). For a venue Kolab that name is copied from
`business_profiles.primary_venue->>'city'`, which is Google Places' `addressComponents.locality`
(`GooglePlacesService.php:179`). Google answers in the local vocabulary and, in a metro area, with the
borough: production held `Ciudad de México` and `Cuajimalps` (Google's own misspelling of the
Cuajimalpa alcaldía). `GET /api/v1/cities` and the app's picker offer the canonical `cities.name`,
which for the same place is `Mexico City` (`CitySeeder::getInternationalCities()`, BE-NF-55). Every
comparison was literal:

| Site | Before | After |
|------|--------|-------|
| `Kolab::scopeForCity()` | `where('preferred_city', $city)` | `LOWER(preferred_city) IN (…every known spelling)` |
| `KolabService::applyFilters()` | `where('preferred_city', $filters['city'])` | `->forCity()` |
| `DiscoveryOpportunityService::applyCommonFilters()` | `where('preferred_city', $filters['city'])` | `->forCity()` |
| `DiscoveryOpportunityService::buildScoreExpression()` | `CASE WHEN preferred_city = ? THEN 40` | `CASE WHEN LOWER(preferred_city) IN (…) THEN 40` |
| `DiscoveryOpportunityService::resolveLocationScore()` (the `city_match` reason) | `$kolab->preferred_city === $viewerCity` | `CityResolver::sameCity()` |
| `DiscoveryOpportunityService::applyMultiKolabRoleFilters()` | `where('city', $filters['city'])` on the event | `LOWER(city) IN (…)` |
| `BusinessVenueService::resolveCity()` | `LOWER(name) = LOWER($cityName)` | `CityResolver::resolve()` |
| `PublicKolabFeedService::cities()` | stored names ∩ `cities.name` | stored names resolved → canonical, then deduped |

**New code:** `App\Services\CityResolver` (singleton, registered in `AppServiceProvider::register()`)
holds one lookup table built from `cities.name` plus `config/cities.php`'s alias map — the local
spellings (`Ciudad de México`, `CDMX`, `Warszawa`, `Séville`…) and the 16 CDMX alcaldías, which *are*
Mexico City. Matching is accent- and case-insensitive (`Str::ascii()` + lowercase + punctuation
stripped), so `Málaga` → `Malaga` needs no alias entry.

- **Write path:** `KolabService::canonicalizePreferredCity()` runs after the venue/community enrichment
  in both `create()` and `update()`, so a new Kolab stores `Mexico City`.
  `OnboardingService` already routed through `BusinessVenueService::resolveCity()`, so a business that
  picks a CDMX venue now also gets `business_profiles.city_id` — it was silently `NULL` before, which
  is why both live CDMX businesses have no city FK.
- **Read path:** the filter still matches the old spellings, so rows written before this change (or by
  an older client) stay reachable without a migration.
- **Backfill:** `php artisan cities:canonicalize [--apply]` (dry-run by default) rewrites
  `kolabs.preferred_city`, `business_profiles.city_name/city_id/city_country` and
  `multi_kolab_events.city`. Production dry run on 2026-09-22: 4 rows (2 Kolabs, 2 business profiles).

**Knock-on:** §31's admin city filter and city map count by `businessProfile.city_id` /
`communityProfile.city_id`. Both live CDMX businesses had `city_id = NULL`, so they were missing from
those counts too; the backfill fixes that surface at the same time, without a change to its code.

**Tables/columns touched:** none added. `kolabs.preferred_city`, `business_profiles.city_id`,
`business_profiles.city_name`, `business_profiles.city_country`, `multi_kolab_events.city` — all
existing columns, values only.

**Tested:** `tests/Unit/Services/CityResolverTest.php` (9),
`tests/Feature/Api/V1/KolabCityFilterTest.php` (6 — browse + discovery find the Google spelling, an
unknown city still matches only itself, create/update store the canonical name),
`tests/Feature/Console/CanonicalizeStoredCitiesTest.php` (2).

**Key invariant:** visibility only. No gate moved, no paywall touched, and the role scope in
`applyRoleScope()` is untouched — a business still sees only `community_seeking`, a community only
`venue_promotion`/`product_promotion` (§3.3, §2.7). A city the alias table does not know is stored and
matched exactly as typed, so an unknown city can never silently collapse into a known one.

---

> **Sync note:** §33 (admin full onboarding, BE-NF-64) and §34 (admin sales mailing, BE-NF-65)
> exist in `kolabing-v2/docs/ROLES-BACKEND-DB-MAP.md` and are not mirrored here yet — both are
