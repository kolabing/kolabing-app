...
**Last updated:** 2026-07-13 (added §0 item 11 — **legal consent gate** (Terms + Privacy): `accepted_terms` required on `POST /auth/register/*`, `terms` block on `GET /auth/me`, `POST /me/consent` re-accept; mirrored from `kolabing-v2` #84/#85, app ticket #67; previous 2026-06-28: added §11.1 gamification mission system v1 — `challenges.app_visible` + the three event/general mission filter sites, mirrored from `kolabing-v2` #49; completion-flow simplification PR 1: `/complete` gate moved from `collaboration_feedback` to a new lightweight `collaboration_completions` table — §0 item 10, §3, §9, §10; previous: kolab single-source-of-truth migration in flight; goal-based onboarding backend fields; planned community verification — see §17)
...
11. ✅ **NEW — legal consent gate (Terms + Privacy), all roles** (2026-07-13, mirrored from `kolabing-v2` #84/#85; app ticket #67). `POST /api/v1/auth/register/{business|community|attendee}` require **`accepted_terms: true`** (missing/`false` → `422 accepted_terms`). `POST /api/v1/auth/{google|apple}` are unchanged — consent is recorded server-side on new-account creation. `GET /api/v1/auth/me` → `data.user.terms` block (`current_version`, `accepted_version`, `accepted_at`, `needs_acceptance`). `POST /api/v1/me/consent` (auth, empty body) re-accepts the current version and returns the refreshed block with `needs_acceptance: false`. **This is a legal gate, NOT the paywall** — it applies to every role including free Communities; never wire `hasActiveSubscription()` into it, and never hardcode the version (read `terms.current_version`). App side: `UserTerms` (`lib/features/auth/models/user_model.dart`), `TermsConsentCheckbox`, `ReconsentGate`, `AuthService.acceptTerms()`. Full contract: [`mobile-consent-flow.md`](./mobile-consent-flow.md).
