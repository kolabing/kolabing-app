...
**A2 — the QR carries a link, and the short code is readable.** Look at what is on screen.
*Expect:* under the QR, a short **6-character code** (e.g. `K7Q2MX`) in large spaced type. Point a **plain phone camera** (not the app) at the QR: it should offer to open a `…/checkin/K7Q2MX` web page. That is the canonical shape — `App\Support\CheckinLink` picks a URL with the short code so the QR stays version 3 and scans across a room instead of only up close.

**A3 — reopening does not retire the code.** Note the code, leave the screen, and open it again.
*Expect:* the **same** code. `CheckinService::openDoor` is idempotent on purpose: a host reopening this on a phone must not kill a QR a laptop is showing to a queue.

**A4 — refresh deliberately rotates.** Tap the refresh icon (top right).
*Expect:* a **different** code. This is the "retire a leaked code" action, and it sends `rotate: true`. Pair with **C2**.

**A5 — copy code.** Tap **Copy token**. *Expect:* a confirmation snackbar, and the clipboard holds the short code.

**A6 — not the organizer.** Sign in as a *different* community leader and open the same event's QR action (or hit the route directly).
...
**A7 — no dead ends.** Confirm there is **no** "View check-ins" button on this screen.
...
**C2 — a rotated/invalid code.** Photograph the QR, have the leader tap refresh (§A4), then scan the **old** code.
...
**C2b — a code shown somewhere other than the app.** If the web panel (or a printed sheet) can show the same event's check-in QR, scan **that** with the app.
*Expect:* a normal check-in. Both surfaces emit the same `…/checkin/{code}` shape and the app reads it by path, so a code is a code regardless of what displayed it. *(This is what the app used to get wrong: it encoded a bare token and rejected the canonical link.)*
...
| **J8** A `…/checkin/{code}` link from the web panel | check-in flow (§B) |

**J9 — dev vs prod codes.** If you have both builds: a QR generated by the prod build must be readable by the prod build. (Codes are matched on their **path**, not the host, so a dev build can read a prod profile QR too — that is intended; the API call behind it will simply be scoped to whichever backend the build points at.)
