...
void main() {
  ...
  test('myStatsProvider recomputes when the authenticated attendee changes', () async {
...
class _ScriptedStatsService extends StatsService {
  ...
  Future<GamificationStats> getMyStats() async {
    final response = responses[callCount < responses.length ? callCount : responses.length - 1];
    callCount += 1;
    return response;
  }
}
