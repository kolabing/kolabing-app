...
>
> Last updated: 2026-06-02 (community-member-flow: NF-6 Community Members + tiers, NF-7 Community Premium, NF-8 Enterprise managed-partnerships; merged master — feedback/completion gate SHIPPED on backend, IF-5/IF-2 contract in docs/api/2026-06-01-feedback-completion-gate-backend-shipped.md)
...
| NF-6 | **Community Members + customisable tiers** | Generalise the `attendee` user type into "Community Members" (UI label only; backend `attendee` enum wire value unchanged). A Community Leader (the `community` user type) gets a new **Community** bottom-nav tab to create + manage **one community free** (roster + member tiers). Each leader defines their OWN tiers (name, rank, colour, perks/permissions) and an **assignment rule** per tier: `manual`, `xp_threshold`, `tenure` (time in community), or `events_attended` (check-ins to this community's events). Community-agnostic by design — Greek life, fitness, running clubs, business comms all map onto it. Backend (kolabing-v2, none built): new `community_members` pivot (member↔community + tier + admin capability + status + joined_at), per-community `community_tiers` (rule type + threshold + perks), auto-assignment job, tier-aware policies, roster/tier CRUD endpoints. App: Community tab + roster/tier management + member-facing tier display; reuse existing gamification (XP/badges/leaderboard/check-in). Branch: `community-member-flow`. | Scoping |
| NF-7 | **Community Premium plan** | Monetisation for the community side (today only businesses pay). Free = **one** community per leader. Premium (paid) = create **multiple** communities + advanced community-management features (TBD: deeper tier customisation, analytics, broadcast, etc.). New subscription product + paywall on the community side, distinct from the Business €29/mo. Pairs with NF-6. | Not started |
